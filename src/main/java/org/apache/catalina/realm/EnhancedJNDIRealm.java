 ...
 *         search pattern configured by the <code>groupSearch</code>
 *         property.</li>
 *     <li>The <code>groupSearch</code> pattern optionally includes pattern
 *         replacements "{0}" for the distinguished name, and/or "{1}" for
 *         the username, of the authenticated user for which groups will be
 *         retrieved.</li>
 *     <li>The <code>groupBase</code> property can be set to the element that
 *         is the base of the search for matching groups.  If not specified,
 *         the entire context will be searched.</li>
 *     <li>The <code>groupSubtree</code> property can be set to
 *         <code>true</code> if you wish to search the entire subtree of the
 *         directory context.  The default value of <code>false</code>
 *         requests a search of only the current level.</li>
 *     <li>The <code>pathname</code> property is used to set the XML file
 *         containing the role to groups mappings.  The default value is
 *         <code>conf/role-to-groups.xml</code>.</li>
 *     </ul></li>
 *
 * <li>In addition, groups may be represented by the values of an attribute
 * in the user's element whose name is configured by the
 * <code>userGroupName</code> property.</li>
 *
 ...
 * @author Ron Rickard
 * @version $Revision: 543693 $ $Date: 2007-06-02 03:42:17 +0200 (sam., 02 juin 2007) $
 * @see org.apache.catalina.realm.JNDIRealm
 */

public class EnhancedJNDIRealm extends RealmBase {
    ...
    protected static final String info =
        "org.apache.catalina.realm.EnhancedJNDIRealm/1.0";


    /**
     * The role to groups mappings.
     */
    private RoleToGroupsMappings mappings;


    /**
     * Descriptive information about this Realm implementation.
     */
    protected static final String name = "EnhancedJNDIRealm";


    /**
     * The pathname (absolute or relative to Catalina's current working
     * directory) of the XML file containing the role to groups mapping
     * information.
     */
    private String pathname = "conf/role-to-groups.xml";
    ...
    /**
     * The base element for group searches.
     */
     ...
     */
    protected MessageFormat groupFormat = null;
    ...
    /**
     * The message format used to select groups for a user, with "{0}" marking
     * the spot where the distinguished name of the user goes.
     */
    protected String groupSearch = null;


    /**
     * Should we search the entire subtree for matching memberships?
     */
    protected boolean groupSubtree = false;
    ...
    /**
     * Return the pathname of our XML file containing role to groups mappings.
     */
    ...
    /**
     * Set the pathname of our XML file containing role to groups mappings. If a
    ...
    /**
     * Return the user group name attribute name for this Realm.
     */
    ...
    /**
     * Set the user group name attribute name for this Realm.
     *
     * @param userGroupName The new userGroup name attribute name
     */
    ...
    /**
     * Return the base element for group searches.
     */
    ...
    /**
     * Set the base element for group searches.
     *
     * @param groupBase The new base element
     */
    ...
    /**
     * Return the message format pattern for selecting groups in this Realm.
     */
    ...
    /**
     * Set the message format pattern for selecting groups in this Realm.
     *
     * @param groupSearch The new group search pattern
     */
    public void setGroupSearch(String groupSearch) {
        ...
        if (groupSearch == null)
            ...
            groupFormat = new MessageFormat(groupSearch);

    }


    /**
     * Return the "search subtree for groups" flag.
     */
    public boolean getGroupSubtree() {
    ...
    /**
     * Set the "search subtree for groups" flag.
     *
     * @param groupSubtree The new search flag
     */
     ...
     * user's directory entry. If the <code>userGroupName</code>
     * configuration attribute is specified, all values of that
     * attribute are retrieved from the directory entry and mapped to
     * roles.
     *
     * @param context The directory context
    ...
    protected User getUserByPattern(DirContext context,
        ...
        // Retrieve values of userGroupName attribute
        Set<String> roles = new HashSet<String>();
        ArrayList<String> groupDNs = null;

        if (userGroupName != null) {
            groupDNs = addAttributeValues(userGroupName, attrs, groupDNs);
            for (String groupDN : groupDNs) {
                roles.addAll(mappings.getRoles(groupDN));
            }
        }

        return new User(username, dn, password, new ArrayList<String>(roles));
    }
    ...
    protected List<String> getRoles(DirContext context, User user)
        ...
        if (user.roles == null) {
            ...
            set = new HashSet<String>(user.roles);
        }

        // Are we configured to do group searches?
        if (groupFormat == null)
            return (new ArrayList<String>(set));

        // Set up parameters for an appropriate search
        String filter = groupFormat.format(new String[] { doRFC2254Encoding(dn), username });
        ...
        NamingEnumeration results =
            context.search(groupBase, filter, controls);
        if (results == null)
            return (new ArrayList<String>(set));  // Should never happen, but just in case ...
        while (results.hasMore()) {
            SearchResult result = (SearchResult) results.next();
            String groupDN = result.getNameInNamespace();
            set.addAll(mappings.getRoles(groupDN));
        }

        if (containerLog.isTraceEnabled()) {
            if (set != null) {
                containerLog.trace("  Returning " + set.size() + " roles");
                for (String role : set) {
                    containerLog.trace(" Found role " + role);
                }
            } else {
    ...
    protected Hashtable getDirectoryContextEnvironment() {
        ...
        if (derefAliases != null)
            env.put(EnhancedJNDIRealm.DEREF_ALIASES, derefAliases);
    ...
    public void start() throws LifecycleException {
        ...
        // Get the role to groups mappings.
        mappings = RoleToGroupsMappings.newInstance(pathname);
    }
